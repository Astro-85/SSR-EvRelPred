...
class VsituDS(Dataset):
    ...
    def read_files(self, split_type: str):        
        ...
        self.master_vseg_lst = read_file_with_assertion("/home/andrew/codebases/VidSitu_full/data/hero_feats/preprocessed/vid_ids.json")
        ...
        # Read video features for HERO
        self.hero_vfeat_db = torch.load(self.full_cfg.hero.vsitu_frm_feats_hero_dir) 
    ...
    def get_vb_arg_data(self, vid_seg_ann_lst: List, is_evrel: bool = False):
        ...
        evrel_seq_lst_all_ev_full_sentence = []
        for ev in range(1, 6):
            only_vb_lst = []
            seq_lst = []
            seq_lst_lens = []
        ...
        def make_full_seq(s1_new, s2_new, s3_new, s4_new, s5_new):
            return(s1_new + evrel_wvoc.sep_token + 
        ...
        if is_evrel:
            ...
            for evix in [0, 1, 3, 4]:
                ...
                for vix in range(len(vid_seg_ann_lst)):
                    ...
                    # FULL SEQUENCE ROBERTA
                    s1_new = get_new_s(evrel_seq_lst_all_ev[0][vix])
                    s2_new = get_new_s(evrel_seq_lst_all_ev[1][vix])
                    s3_new = get_new_s(evrel_seq_lst_all_ev[2][vix])
                    s4_new = get_new_s(evrel_seq_lst_all_ev[3][vix])
                    s5_new = get_new_s(evrel_seq_lst_all_ev[4][vix])
                    ...
                    if evix == 0:
                        ...
                        s3_new = "*" + s3_new
                        s5_new = "**" + s5_new
                    new_seq = evrel_wvoc(make_full_seq(s1_new, s2_new, s3_new, s4_new, s5_new))["input_ids"]
            ...
            out_dct["evrel_seq_out_lens"] = torch.tensor(out_evrel_seq_by_ev_lens).long()
            ...
            full_out_evrel_seq_one_by_ev = []
            full_out_evrel_seq_onelens_by_ev = []
            ...
            for evix in [0, 1, 2, 3, 4]:
                ...
                full_out_evrel_seq_one_lst = []
                full_out_evrel_seq_onelens_lst = []

                out_evrel_vbonly_one_lst = []
                out_evrel_vbonly_onelens_lst = []
                full_out_evrel_vbonly_one_lst = []
                full_out_evrel_vbonly_onelens_lst = []
                for vix in range(len(vid_seg_ann_lst)):
                    # BASLINE TWO INPUT ROBERTA
                    ...
                    full_out_evrel_seq_one_lst.append(new_seq_pad.tolist())
                    full_out_evrel_seq_onelens_lst.append(new_seq_msk)

                out_evrel_seq_one_by_ev.append(out_evrel_seq_one_lst)
                out_evrel_seq_onelens_by_ev.append(out_evrel_seq_onelens_lst)
                full_out_evrel_seq_one_by_ev.append(full_out_evrel_seq_one_lst)
                full_out_evrel_seq_onelens_by_ev.append(full_out_evrel_seq_onelens_lst)
            ...
            out_dct["evrel_seq_out_ones_lens"] = torch.tensor(out_evrel_seq_onelens_by_ev).long()
            out_dct["rob_full_evrel_seq_out_ones"] = torch.tensor(full_out_evrel_seq_one_by_ev).long()
            out_dct["rob_full_evrel_seq_out_ones_lens"] = torch.tensor(full_out_evrel_seq_onelens_by_ev).long()
            out_dct["evrel_vbonly_out_ones"] = torch.tensor(out_evrel_vb_one_by_ev).long()
            out_dct["evrel_vbonly_out_ones_lens"] = torch.tensor(out_evrel_vb_onelens_by_ev).long()   
    ...
    def get_frm_feats_all(self, idx: int):
        ...
        # Added feature loader for HERO
        hero_vid_feats = self.hero_vfeat_db[vid_seg_name]
        master_idx = torch.tensor([self.master_vseg_lst.index(vid_seg_name)])

        return {"frm_feats": vid_feats, "hero_frm_feats": hero_vid_feats, "vid_idx": master_idx}
