...
from utils.trn_utils_pretraining import (
    progress_bar,
    move_to,
    synchronize,
    is_main_process,
...
from vidsitu_code.evl_fns_pretraining import EvlFn_Vb, EvalFnCap, EvlFn_EvRel
    ...
    def forward(self, model, loss_fn, dl, dl_name, rank=0, pred_path=None, mb=None):
        ...
        if is_main_process():
            ...
            if self.full_cfg.only_test:
                ...
                spl = "valid"
            out_acc = self.evl_fn(fname, gt_lst, split_type=spl)
            val_acc = {
...
class EvalB_Acc(EvalB):
    ...
    def forward_one_batch(self, mdl, inp):
        ...
        # Generates prediction files from model outputs
        def get_dct(pred_vbs, pred_scores, ann_idx):
            pred_vbs_out = []
            pred_scores_out = []
            ...
            # iterate over Ev1-5
            for pvb, pvs in zip(pred_vbs, pred_scores):
                viscom_dct = {0: "Null", 1: "Before", 2: "After", 3: "Intent"}
                pvb_used = [pvb_i[0] for pvb_i in pvb]
                pvb_str = [viscom_dct[pv] for pv in pvb_used]
                #print("\nself.comm.evrel_dct_opp: \n", self.comm.evrel_dct_opp)
                #print("\npvb_str: \n", pvb_str)
