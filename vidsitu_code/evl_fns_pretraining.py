...
def read_gt_file(full_cfg, task_type, split_type):
    ds_cfg = full_cfg.ds.viscom
    split_files_cfg = ds_cfg.split_files_lb
    vsitu_ann_files_cfg = ds_cfg.viscom_ann_files_lb
...
class EvlFn_EvRel:
    ...
    def simple_acc_evrel(self, pred_file: str, gt_lst: list, split_type: str = "valid"):
        #print("\ngt_lst\n", gt_lst)
        
        hypos_gts_mask = self.prepare_hyp_gts(
            gt_lst, pred_file=pred_file, split_type=split_type
        )
    ...
    def prepare_hyp_gts(self, gt_lst: list, pred_file: str, split_type: str = "valid"):
        ...
        for ann_idx in hypo_dct:
            if ann_idx not in hypos:
                ...
                '''
                ba_dct = {0: "Null", 1: "Before", 2: "After", 3: "Intent"}
                ...
                for tnsr in gt_lst:
                    tnsr = tnsr.tolist()
                    for gt_seq in tnsr:
                        flat_gt_lst.append(gt_seq)
                #print("\nflat_gt_lst: \n", flat_gt_lst)
                
                
                gt1 = ba_dct[flat_gt_lst[ann_idx][0]]
                gt2 = ba_dct[flat_gt_lst[ann_idx][1]]
                gt3 = ba_dct[flat_gt_lst[ann_idx][2]]
                gt4 = ba_dct[flat_gt_lst[ann_idx][3]]
                gt_vbs = [
                    [gt1, gt1, gt1],
                    [gt2, gt2, gt2],
                    [gt3, gt3, gt3],
                ...
                hypos[pred_one["ann_idx"]] = {
                    ev_i: [preds[ev_ix][0], preds[ev_ix][0], preds[ev_ix][0]] for ev_ix, ev_i in enumerate(ev_lst)
                }
                ...
                mask[pred_one["ann_idx"]] = {"Ev1": 1, "Ev2": 1, "Ev4": 1, "Ev5": 1}
